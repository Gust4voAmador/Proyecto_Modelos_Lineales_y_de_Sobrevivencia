...
dif_count <- ggplot(data_plot, aes(x = `Differential Count`, fill = `Differential Count`)) +
  ...
  theme(
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5), # Título centrado y más grande
    axis.title = element_text(size = 14, face = "bold"),
    axis.text = element_text(size = 11),
    strip.text = element_text(size = 14, face = "bold"),
    legend.text = element_text(size = 11),
...
tabla_resumen <- data_clean %>%
  drop_na(Age, `Dengue Diagnosis`) %>%  # elimina filas con NA en las columnas relevantes
  group_by(`Dengue Diagnosis`) %>%
  summarise(
